...
const open = async (row) => {
  ...
  if (row.id) {
    //需要基于 row.id 发送请求，获取对应的详细数据，进行回写
    // console.log('编辑回显')
    ...
    //这里重置了表单的数据，但是图片和富文本编辑器需要手动重置
    imgUrl.value = ''

    //使用nextTick，因为调用open时Dom还未渲染完成，无法挂载ref
    nextTick(() => {
  ...
  >
    ...
    <el-form :model="formModel" ref="formRef" label-width="100px">
      ...
      <el-form-item label="文章封面" prop="cover_img">
        <!-- 需要关闭element-plus 的自动上传，不需要配置 action 等参数
              只需做耆那端的本地预览图片，无需上传
